...
export default function JejaringMap({
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      map.loadImage("/icons/pin-red.png", (err, image) => {
        ...
        /* =====================================================
           MASK GLOBAL (GELAPKAN SEMUA)
        ===================================================== */
        map.addSource("mask-global", {
          type: "geojson",
          data: {
            type: "Feature",
            geometry: {
        ...
        /* =====================================================
           KECAMATAN (TERANGKAN AREA JAGAKARSA)
        ===================================================== */
        ...
        map.addLayer({
          ...
          paint: {
            "fill-color": "#000",
            "fill-opacity": 0, // ⬅️ INI YANG MEMBUAT HOLE AMAN
          },
        });
  ...
  useEffect(() => {
    ...
    if (!map || !map.getSource("jejaring")) return;
  ...
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !map.getLayer("kelurahan-fill")) return;
